...
ignore_patterns = [r'^\d+[a-z]{0,2}$', r'^[a-z]$', r'[^\x00-\x7F]', r'(.)\1{2,}'] #[r'^[\.\_\^]+$', r'//{0,1}.+', r'\\.+', r'^={3,}$', r'\d{4}-\d{2}-\d{2}', r'^www\.', r'^[a-z]$', r'\d{2}:\d{2}:\d{2}', r'[^\x00-\x7F]']

synonym_refs = {}

with open(dirname(__file__) + '/keras/programming_synonyms.csv', 'r') as programming_synonyms_file:
...
def tokenize_body(body):
    ...
    no_hyphens = expanded_html.replace('-', ' ')

    body_tokens = text_to_word_sequence(no_hyphens)

    stripped_tokens = map(lambda token: token.strip('\'"').replace('\'s', ''), body_tokens)

    filtered_tokens = filter(lambda token: token not in ignore_tokens, stripped_tokens)
...
def preprocess_text(question, use_keras=True):
    ...
    no_hyphens = expanded_question.replace('-', ' ')

    # Tokenize the word into its keywords
    if not use_keras:
        title_tokens = nltk.word_tokenize(no_hyphens)
    else:
        title_tokens = text_to_word_sequence(no_hyphens)

    stripped_tokens = map(lambda token: token.strip('\'').strip('"'), title_tokens)

    # Lowercase tokens
    lowercase_tokens = map(lambda token: token.lower(), stripped_tokens)
